...
class Interval(EmbeddedDocument):
    ...
    period = StringField(choices=PERIODS, required=True)
...
class PeriodicTask(DynamicDocument):
    ...
    def clean(self):
        ...
        if not (self.interval or self.crontab):
            ...
            raise ValidationError(msg)
        if not isinstance(self.expires, NoneType) and not isinstance(self.expires,DateTimeField) and not isinstance(self.expires, IntField):
            msg = 'Expires: Only datetime or int.'
            raise ValidationError(msg)
